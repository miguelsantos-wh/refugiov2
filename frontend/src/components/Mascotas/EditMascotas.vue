              ...
              <div class="form-group row">
                ...
                <div class="col-sm-6">
                  <div v-for="vacuna in form.vacunas_total" :key="vacuna.id">
                    <label>
                      <input type="checkbox" :value="vacuna.id" v-model.trim="form.vacuna">
                      {{ vacuna.nombre }}
                    </label>
                  </div>
                </div>
  ...
  methods: {
    onSubmit(evt) {
      ...
      axios.put(path, this.form).then((response) => {
        swal("Mascota actualizada correctamente", "", "success")
            .then(() => {
              console.log(response.data);
              location.href = "/mascotas";
            });
      }).catch((error) => {
        swal(`Mascota no actualizada`, "", "error")
    ...
    getMascota() {
      ...
      axios.get(path).then((response) => {
        ...
        this.form.edad_aproximada = response.data.edad_aproximada
        this.form.fecha_rescate = response.data.fecha_rescate
        this.form.persona = response.data.persona.id
        this.form.vacuna = response.data.vacuna.map(vacuna => vacuna.id)
        this.getPersonas()
        this.getVacunas()
        console.log(response.data)
      }).catch((error) => {
